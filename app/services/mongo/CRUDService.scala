...
trait CRUDService[E, ID] {
  ...
  def bulkInsert(listBuffer:Traversable[E]):Future[MultiBulkWriteResult]
}

import models.models.Identity
...
abstract class MongoCRUDService[E: Format, ID: Format](implicit identity: Identity[E, ID])
  ...
  def getNumberOfElements():Future[Int]=collection.count()
  ...
  override def bulkInsert (listBuffer : Traversable[E]):Future[MultiBulkWriteResult] = {
    val streamJsObj:Stream[JsObject] = listBuffer.toStream.map(book => Json.toJson(book).as[JsObject])
    collection.bulkInsert(streamJsObj,true)
  }

  def updateBySelectorAndModifier(selector: JsObject, modifier: JsObject):Future[UpdateWriteResult]={
